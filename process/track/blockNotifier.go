...
type blockNotifier struct {
	...
	notarizedHeadersHandlers    []func(headers []data.HeaderHandler, headersHashes [][]byte)
}
...
func NewBlockNotifier() (*blockNotifier, error) {
	bn := blockNotifier{}
	bn.notarizedHeadersHandlers = make([]func(headers []data.HeaderHandler, headersHashes [][]byte), 0)
	return &bn, nil
}

// CallHandlers calls all the registered handlers to notify them about the new received headers
func (bn *blockNotifier) CallHandlers(headers []data.HeaderHandler, headersHashes [][]byte) {
	if len(headers) == 0 {
		return
	}
...
func (bn *blockNotifier) RegisterHandler(handler func(headers []data.HeaderHandler, headersHashes [][]byte)) {
	if handler == nil {
