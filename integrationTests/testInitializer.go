...
func CreateSimpleGenesisBlock() *dataBlock.Header {
...
// CreateGenesisBlocks creates empty genesis blocks for all known shards
func CreateGenesisBlocks() map[uint32]data.HeaderHandler {
	genesisBlocks := make(map[uint32]data.HeaderHandler)
	genesisBlocks[0] = CreateSimpleGenesisBlock()
	return genesisBlocks
}
...
func MakeDisplayTable(nodes []*TestProcessorNode) string {
	header := []string{"pk", "shard ID", "txs", "miniblocks", "headers", "connections"}
...
func GenerateSkAndPkInShard() (crypto.PrivateKey, crypto.PublicKey, crypto.KeyGenerator) {
...
func CreateSendersAndReceiversInShard(
	...
	for i := uint32(0); i < nbSenderReceiverPairs; i++ {
		sendersPrivateKeys[i], _, _ = GenerateSkAndPkInShard()
		_, receiversPublicKeys[i], _ = GenerateSkAndPkInShard()
	}
...
func generateValidTx(
	...
	skSender, pkSender, _ := GenerateSkAndPkInShard()
	pkSenderBuff, _ := pkSender.ToByteArray()

	_, pkRecv, _ := GenerateSkAndPkInShard()
...
// SetupSyncNodesOneShard creates nodes with sync capabilities divided into one shard
func SetupSyncNodesOneShard(
...
func EmptyDataPools(nodes []*TestProcessorNode) {
	for _, n := range nodes {
		emptyNodeDataPool(n)
	}
}
