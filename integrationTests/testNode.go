...
import (
	...
	p2pRating "github.com/multiversx/mx-chain-p2p-go/rating"
...
func CreatePkBytes() map[uint32][]byte {
	pk := []byte("afafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafafaf")
	pksbytes := make(map[uint32][]byte, 1)
	pksbytes[0] = pk
	return pksbytes
}

func newBaseTestProcessorNode() *TestProcessorNode {
	shardCoordinator := &sharding.OneShardCoordinator{}

	kg := &mock.KeyGenMock{}
	sk, pk := kg.GeneratePair()

	pksBytes := CreatePkBytes()
	...
	nodesSetup := &mock.NodesSetupStub{
		InitialNodesInfoCalled: func() (m map[uint32][]nodesCoordinator.GenesisNodeInfoHandler, m2 map[uint32][]nodesCoordinator.GenesisNodeInfoHandler) {
			...
			oneMap[0] = append(oneMap[0], mock.NewNodeInfo(address, pksBytes[0], 0, InitialRating))
			return oneMap, nil
		},
	...
	nodesCoordinatorStub := &shardingMocks.NodesCoordinatorStub{
		...
		GetAllValidatorsPublicKeysCalled: func() (map[uint32][][]byte, error) {
			...
			keys[0] = append(keys[0], pksBytes[0])
...
func NewTestProcessorNode() *TestProcessorNode {
